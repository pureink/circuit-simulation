...
//被注释掉的部分是我的思路
function readIModules(json_text) {
    const obj = JSON.parse(json_text);
    return obj.modules;
}
function readIWires(json_text) {
    const obj = JSON.parse(json_text);
    return obj.wires;
}
...
//     return p;
// }
function finalPorts(modules, wires) {
...
//生成函数
function getAllMdu(json_text) {
    const imdu = readIModules(json_text);
    const iwr = readIWires(json_text);
    let wires = readWires(iwr);
    let mdu = readModules(imdu);
...
function main() {
    ...
    //修改对象名称
    function cvtPortName(before, name1, name2) {
